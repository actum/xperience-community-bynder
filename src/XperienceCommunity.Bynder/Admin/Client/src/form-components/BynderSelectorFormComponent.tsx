...
export const BynderSelectorFormComponent = (
  props: BynderSelectorClientProps
) => {
  const initAssets = props.value ?? [];

  const [isOpen, setIsOpen] = React.useState<boolean>(false);
  const [selectedAssets, setAssets] = React.useState<Asset[]>(initAssets);
  const [selectedBynderAssets, setBynderAssets] = React.useState<string[]>(
    initAssets.map((i: { databaseId: any }) => i.databaseId)
  );

  const bynderAllowedTypes: assetType[] = props.allowedTypes || [];
  ...
  const onRemoveItem = (key: any) => {
    var assets = selectedAssets.filter((item) => {
      return item.id !== key;
    });
    selectAssets(assets);
  };
